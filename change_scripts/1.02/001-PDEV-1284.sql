  ...
  BEGIN
    BEGIN
      ...
      IF(p_indicator !~ '^[a-zA-Z0-9_]*$') THEN
          RAISE EXCEPTION 'Parameter p_indicator can only contains alphanumeric and underscore' USING HINT = 'indicator_bad_format';
      END IF;
      ...
      IF(NOT EXISTS(SELECT 1
          ...
          RAISE EXCEPTION 'Indicator function "%" does not exist', p_indicator USING HINT = 'indicator_does_not_exist';
      END IF;
      ...
      IF(NOT EXISTS(SELECT 1
          ...
          RAISE EXCEPTION 'Clinic reference "%" not found', p_clinic USING HINT = 'clinic_does_not_exist';
      END IF;
      ...
      IF(p_provider IS NOT NULL AND NOT EXISTS(SELECT 1
          ...
          RAISE EXCEPTION 'Provider identifier "%" not found', p_provider USING HINT = 'provider_does_not_exist';
      END IF;
    ...
    EXCEPTION WHEN others THEN
      ...
      GET STACKED DIAGNOSTICS v_err_message = MESSAGE_TEXT, v_err_hint = PG_EXCEPTION_HINT;
      ...
      IF(v_err_hint IN ('indicator_bad_format', 'indicator_does_not_exist', 'clinic_does_not_exist', 'provider_does_not_exist')) THEN
        RAISE EXCEPTION '%', v_err_message;
      ELSE
        --Pass generic error information back to the client.
        RAISE EXCEPTION 'Error occured in api.aggregate(). See audit.aggregate_log.';
      END IF;
    END;
  END;
$BODY$
